...
void APP_Initialize(void) {
    ...
    OC1R = 1200; // initialize before turning OC1 on; afterward it is read-only
    OC4CONbits.OCM = 0b110; // PWM mode without fault pin; other OCxCON bits are defaults
    OC4RS = 1200; // duty cycle
    OC4R = 1200; // initialize before turning OC4 on; afterward it is read-only
...
//              i++;
                ...
                error = rxVal - 320; // 240 means the dot is in the middle of the screen
                    ...
                    OC4RS = 400+2*error; // velocity, 50%
                    OC1RS = 400-2*error; // turn left
                    }
