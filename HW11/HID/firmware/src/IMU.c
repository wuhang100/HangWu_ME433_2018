...
float read_z(unsigned char * data) { 
    float value;
    short int z = data[13]<<8 | data[12];
    value = ((float)z/(float)16000);
    return value;
}

float read_x0(unsigned char * data) { 
    float value;
    short int xx = data[3]<<8 | data[2];
    value = ((float)xx/(float)16000);
    return value;
}

float read_y0(unsigned char * data) { 
    float value;
    short int yy = data[5]<<8 | data[4];
    value = ((float)yy/(float)16000);
    return value;
}

float read_z0(unsigned char * data) { 
    float value;
    short int zz = data[7]<<8 | data[6];
    value = ((float)zz/(float)16000);
    return value;
}


short int raw_z(unsigned char * data) { 
    short int z = data[13]<<8 | data[12];
    return z;
}

short int raw_x(unsigned char * data) { 
    short int x = data[9]<<8 | data[8];
    return x;
}

short int raw_y(unsigned char * data) { 
    short int y = data[11]<<8 | data[10];
    return y;
}