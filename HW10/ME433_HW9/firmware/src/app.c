...
int read_go=0;
short int rawz;
int maf[4]; int fil_maf;
int fil_iir;
int fir[4]; int fil_fir;
float fil_temp;
        ...
        case APP_STATE_SCHEDULE_WRITE:
            ...
            /* THIS IS WHERE YOU CAN READ YOUR IMU, PRINT TO THE LCD, ETC */
                ...
                if (read_go==1){
                    if (j<=100){
                       if (j==1){
                        ...
                        fil_fir = rawz;
                        maf[0] = rawz;
                        len = sprintf(dataOut, "%d %d %d %d %d;\r\n",j,rawz,fil_maf,fil_iir,fil_fir);
                        }                   
                    if (j==2){
                        maf[1] = rawz;
                        fil_maf = rawz;
                        fil_temp = 0.3*((float)(maf[0]))+0.7*((float)rawz);
                        fil_iir = (int)fil_temp;
                        fil_fir = rawz;
                        len = sprintf(dataOut, "%d %d %d %d %d;\r\n",j,rawz,fil_maf,fil_iir,fil_fir);
                        }
                    if(j==3){
                        maf[2] = rawz;
                        fil_maf = rawz;
                        fil_temp = 0.3*((float)(maf[1]))+0.7*((float)rawz);
                        fil_iir = (int)fil_temp;
                        fil_fir = rawz;
                        len = sprintf(dataOut, "%d %d %d %d %d;\r\n",j,rawz,fil_maf,fil_iir,fil_fir);                             
                    }
                     if(j==4){
                        maf[3] = rawz;
                        fil_temp =0.25*((float)(maf[0]))+0.25*((float)(maf[1]))+0.25*((float)(maf[2]))+0.25*((float)(maf[3]));
                        fil_maf = (int)fil_temp;
                        fil_temp = 0.3*((float)(maf[2]))+0.7*((float)rawz);
                        fil_iir = (int)fil_temp;
                        fil_temp =0.1*((float)(maf[0]))+0.2*((float)(maf[1]))+0.4*((float)(maf[2]))+0.3*((float)(maf[3]));
                        fil_fir = (int)fil_temp;
                        len = sprintf(dataOut, "%d %d %d %d %d;\r\n",j,rawz,fil_maf,fil_iir,fil_fir);                             
                    }
                     if(j>=5){
                        maf[0]=maf[1];
                ...
                i++;
                    ...
                    LCD_drawString(10,30,str2,BLUE); //String    
                    sprintf(str3,"z_raw: %d",(int)(Gy*16000));
                    LCD_drawString(10,40,str3,BLUE); //String 
                    i=0;
                }
